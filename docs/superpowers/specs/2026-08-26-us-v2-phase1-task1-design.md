...
| 대상 | 소스 | 비고 |
|---|---|---|
| SPY/QQQ/섹터ETF 11종/VIX/VIX9D/VIX3M/TNX/DXY | yfinance | 무료, Phase 1 승인 소스(§17) |
| S&P500 구성종목 리스트 | Wikipedia 스크랩 | Phase 1은 현재 시점 구성만 필요(시점별 구성은 Phase 2, §5-2) |
| S&P500 구성종목 일봉 | yfinance 배치 다운로드 | breadth(%above50DMA, NH-NL) 계산용 |
| SPY/VIX 2차 소스(V5 교차검증) | stooq CSV (`stooq.com/q/d/l/`) | 무료, 키 불필요, requests로 직접 호출 |

## V1~V7 게이트 구현 방식
- V1 스키마 / V3 범위 / V4 연속성(|r|>30%) / V6 커버리지(≥98%): 문서 그대로 구현
- V2 신선도: 최신 봉의 날짜가 오늘 기준 직전 거래일과 일치하는지
- V5 교차검증: SPY·VIX 종가를 stooq와 대조, 편차 >0.1%면 HALT
